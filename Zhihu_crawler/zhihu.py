...
"""
    ...
    def get_board(self) -> list:
        ...
        hot_list = eval(temp)
        for hot in hot_list:
            title = hot["target"]["titleArea"]["text"]
            heat = hot["target"]["metricsArea"]["text"]
            excerpt = hot["target"]["excerptArea"]["text"]
            zh_url = hot["target"]["link"]["url"]
            if len(hot["cardId"].split("Q_")) < 2:
                continue
            q_id = hot["cardId"].split("Q_")[1]
            res_list.append({'title': title, 'heat': heat, 'excerpt': excerpt, 'url': zh_url, 'qid': q_id})
        return res_list
    ...
    def get_question(self, qid: int) -> dict:
        ...
        q_url = "https://www.zhihu.com/question/"+str(qid)
        r = requests.get(q_url, headers=self.settings["headers"])
        soup = BS(r.text, 'lxml')
        script_text = soup.find("script", id="js-initialData").get_text()
        ls = json.loads(script_text)
        created = ls["initialState"]["entities"]["questions"][str(qid)]["created"]
        follower_count = ls["initialState"]["entities"]["questions"][str(qid)]["followerCount"]
        visit_count = ls["initialState"]["entities"]["questions"][str(qid)]["visitCount"]
        answer_count = ls["initialState"]["entities"]["questions"][str(qid)]["answerCount"]
        title = ls["initialState"]["entities"]["questions"][str(qid)]["title"]
        raw = ls["initialState"]["entities"]["questions"][str(qid)]["excerpt"]
        hit_at = time.time()
        dc = {'created': created, 'followerCount': follower_count, 'visitCount': visit_count, 'answerCount': answer_count, 'title':title, 'raw': raw, 'hit_at': hit_at}
        return dc
